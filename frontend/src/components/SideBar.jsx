...
console.log(error)
  ...
  return (
    <div className={`lg:w-[25%] w-full h-full overflow-hidden lg:block bg-slate-200  relative ${!selectedUser?"block":"hidden"}`}>
...
{searchData?.map((user)=>(
     ...
     <h1 className='text-gray-800 font-semibold text-[20px]'>{user.name || user.userName}</h1>
      ...
      <div className='w-full h-[300px] bg-[#ff203e] rounded-b-[30%] shadow-gray-400 shadow-lg flex flex-col justify-center px-[20px] '>
    <h1 className='text-white font-bold text-[25px]'>BlinkTalk</h1>
   <div className='w-full flex justify-between items-center'>
    <h1 className='text-gray-800 font-bold text-[25px]'>{userData.name || "user"}</h1>
